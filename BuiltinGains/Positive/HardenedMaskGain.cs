...
{
    internal class HardenedMaskGain : GainImpl<HardenedMaskGain, HardenedMaskGainData>
    {
        public override GainID GainID => HardenedMaskGainEntry.HardenedMaskGainID;
    }

    internal class HardenedMaskGainData : GainDataImpl
    {
        public override GainID GainID => HardenedMaskGainEntry.HardenedMaskGainID;
    }

    internal class HardenedMaskGainEntry : GainEntry
    {
        public static GainID HardenedMaskGainID = new GainID("HardenedMask", true);
        public static int countdown;
        public static ILCursor c;

        public override void OnEnable()
        {
            GainRegister.RegisterGain<HardenedMaskGain, HardenedMaskGainData, HardenedMaskGainEntry>(HardenedMaskGainID);
        }
        ...
        private static void Spear_HitSomething(MonoMod.Cil.ILContext il)
        {
            ...
            {
                EmgTxCustom.Log("Spear_HitSomething");
                EmgTxCustom.Log(emitCursor.Next.OpCode);
                emitCursor.Emit(OpCodes.Ldarg_0);

                emitCursor.EmitDelegate<Func<SharedPhysics.CollisionResult, Spear, bool>>((result, self) =>
                {
                    EmgTxCustom.Log($"Spear_HitSomething {result.obj}");
        ...
        public static bool CheckIfMaskBounce(Player player, Spear spear, Vector2? directionAndMomentum)
        {
            ...
            {
                EmgTxCustom.Log($"check grasp {i} : {player.grasps[i]?.grabbed}");
                if (player.grasps[i] != null && player.grasps[i].grabbed is VultureMask mask && mask.donned > 0.5f)
                {
                    ...
                    EmgTxCustom.Log($"{playerDir}, {spearDir}, {Vector2.Dot(playerDir, spearDir)}");
